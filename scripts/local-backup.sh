...
mkdir "$backup_target"
docker compose -f "$COMPOSE_FILE" exec -T postgres sh -c 'pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" --format=custom' > "$backup_target/database.dump"
...
if [[ -n "${BACKUP_MEDIA_DIR:-}" ]]; then
  ...
  docker compose -f "$COMPOSE_FILE" exec -T api1 tar -C /var/lib/biobalance/media -czf - . > "$backup_target/media.tar.gz"
fi
