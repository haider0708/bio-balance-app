...
backup_database_bytes=$("${compose[@]}" exec -T postgres sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB" -Atc "SELECT pg_database_size(current_database())"' </dev/null)
if [[ -n "${BACKUP_MEDIA_DIR:-}" ]]; then
  backup_media_kib=$(du -sk "$BACKUP_MEDIA_DIR" | cut -f1)
else
  backup_media_kib=$("${compose[@]}" exec -T api1 du -sk /var/lib/biobalance/media </dev/null | cut -f1)
fi
...
mkdir "$backup_target"
"${compose[@]}" exec -T postgres sh -c 'pg_dump -U "$POSTGRES_USER" -d "$POSTGRES_DB" --format=custom' </dev/null > "$backup_target/database.dump"
...
if [[ -n "${BACKUP_MEDIA_DIR:-}" ]]; then
  ...
  "${compose[@]}" exec -T api1 tar -C /var/lib/biobalance/media -czf - . </dev/null > "$backup_target/media.tar.gz"
fi
