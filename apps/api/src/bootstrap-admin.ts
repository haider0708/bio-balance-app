...
async function main() {
  ...
  try {
    if (await db.user.findFirst({ where: { platformAdmin: true } }))
    ...
    await writeFile(
      output,
      JSON.stringify({ email, password, totpUri: uri }, null, 2),
      { mode: 0o600, flag: "wx" },
    );
    await db.$transaction(async (tx) => {
      ...
      await tx.auditEntry.create({
        data: {
          ...
          targetId: id,
          details: { localProvisioning: true },
        },
      });
    });
    console.log(
      "Administrator created. Import the authenticator URI and store the private setup file securely.",
    );
  } finally {
    await db.$disconnect();
  }
...
void main().catch(() => {
  console.error(
    "Administrator provisioning failed; check configuration and the private setup file.",
  );
  process.exitCode = 1;
});
