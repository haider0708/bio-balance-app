...
Images are fully built before containers start. The default rollback uses application source `a8b4610` with the corrected runtime packaging and the current additive schema; set `ROLLBACK_REF` to another reviewed compatible commit for a future exercise. The final audit uses `584f40f` only as an isolated compatibility fixture, not as a production rollback recommendation. The test samples authorized lot reads once per second while replacing both APIs, then both workers, and checks stock/version on every sample. Full snapshots check stock/points before and after each rollout within the ordinary snapshot rate budget. HTTP failures, including rate limits, fail the exercise; security budgets remain unchanged. Requests and Compose waits are bounded, and a failed probe still waits for the child process to exit. The test verifies image identity, repeats the accepted sale, returns to the current images and checks stock/points/media again.
