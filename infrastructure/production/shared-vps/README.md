...
Les timers `biobalance-backup.timer` et `biobalance-monitor.timer` utilisent `/etc/biobalance/backup.env`. Le moniteur vérifie services, ressources, jobs, fraîcheur et résultat des sauvegardes, copies interrompues/corrompues, santé HTTPS publique et certificat d’origine (>7 jours). Les résultats sont dans journald ; une supervision indépendante/notification externe reste à raccorder. La rotation Docker reste plafonnée à 5 fichiers de 10 Mo par service.
