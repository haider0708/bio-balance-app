...
| Notifications | Audiences, sessions, doublons, workers et boîte interne vérifiés ; modèles d’accès/sécurité testés et déployés ; email réel accepté en TLS en une tentative | Confirmation de réception/classement Gmail par le destinataire ; alertes OS app fermée reportées, sans Firebase |
...
| CI | Les quatre jobs backend, Android, parcours Android/reprise et compilation iOS non signée réussissent sur `c2ccab4` ; [exécution complète](https://github.com/haider0708/bio-balance-app/actions/runs/35746770350) | Aucune pour ce commit ; conserver ces contrôles pour les changements suivants |
