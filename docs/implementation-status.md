...
Il n’y a aucune modification du code serveur exécuté ni de migration à déployer. Les trois copies Android signées et leur installation sont consignées dans `tests/deployment/access-invitation-evidence-2026-09-23.json`. La connexion ADB du Samsung doit être rétablie avant leur installation ; les données et sessions existantes doivent être conservées.
