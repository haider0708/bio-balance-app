...
## 2026-09-23 — Accès du responsable et activation du vendeur, 1.1.6+10

Le compte déjà activé affiche maintenant deux sections distinctes : **Créer mon groupe** pour préparer l’activité, puis **Actualiser mes accès** avec une explication de son rôle. Un vendeur sans magasin reçoit des instructions adaptées et ne voit pas les onglets du réseau administrateur. L’actualisation est protégée contre les appels simultanés et ouvre le groupe ou le magasin nouvellement attribué, sans remplacer un espace déjà ouvert ni restaurer un accès révoqué.

Les invitations d’équipe demandent une confirmation de l’adresse exacte, du rôle et des magasins. Le renvoi explique l’invalidation des anciens codes ; le statut **En attente d’activation** évite de confondre une invitation enregistrée avec une réception d’email. L’annulation conserve le formulaire. La documentation précise activation, connexion et récupération.

Diagnostic réel limité au vendeur signalé : les précédents envois ont été acceptés par SMTP, sans job échoué. Après confirmation de l’adresse par le propriétaire, une nouvelle invitation a été émise par le service métier existant ; l’activation et une session valide avec un magasin autorisé ont été observées ensuite. L’acceptation SMTP seule ne prouve pas le placement en boîte de réception. Aucun reset de base ou d’installation, aucun changement de mot de passe ni de permission effectué pour ce correctif.

Validation : **261 tests Flutter réussis**, quatre tests dépendants de fixtures ignorés, analyse sans anomalie, **16 tests email réussis** sur PostgreSQL isolé, parcours Android complet administrateur → responsable → vendeur réussi avec vérification des effets en base. Les nouvelles régressions couvrent le renvoi remplaçant l’ancien code, l’activation unique, les permissions magasin, le rafraîchissement après attribution et les dispositions portrait/paysage à 200 %. Un premier parcours Android a échoué dans le harnais en sélectionnant un produit masqué par le clavier ; les réceptions/commandes utilisent désormais la recherche prévue par le sélecteur, puis le parcours complet est passé. Revue manuelle de `docs/screenshots/responsible-access-setup.png` effectuée.

Il n’y a aucune modification du code serveur exécuté ni de migration à déployer. Source livrée : `9030d290805f48eeb4205cc213836ee50d92da7b`. Les trois copies Android 1.1.6+10 sont signées, vérifiées indépendamment (empreintes source/fichiers, certificats APK/AAB, package/version) et installées sur le Samsung SM-G975F par USB. Les UID et données restent conservés. Les écrans Vendeur et Responsable ont rouvert avec leurs sessions, groupe/magasin, ventes et points existants ; le propriétaire confirme réception de l’email et ouverture des trois comptes.

La reprise Android réelle est également passée en local : arrêt du processus après commit SQLite, même identifiant/payload et compte après redémarrage, une vente acceptée, stock 7/version 3 et 30 points, caméra refusée avec recherche manuelle, vidéo H.264 téléchargée lue hors ligne. La première tentative CI `35911905152` passe backend, compilations Android/iOS et parcours des trois rôles, mais le pilote Flutter perd son service `getIsolate` au redémarrage du test de reprise. Le job relancé réussit : les quatre jobs CI sont désormais passés. Le premier échec et les passes locales sont conservés séparément dans `tests/deployment/access-invitation-evidence-2026-09-23.json`.

## 2026-09-23 — Plan de raffinement après essai manuel

Le propriétaire confirme que le vendeur garde ses corrections/retours de ventes et demandes de récompenses. La réception revient au responsable. `docs/operations-refinement-plan-2026-09-23.md` définit le prochain périmètre : alertes précises et traitables, suspension/archivage, commandes amendables/annulables, incidents de livraison distincts d’une réception, badge non-lu et vérification complète des scopes/permissions/synchronisation. Il s’agit d’un plan ; ces nouvelles fonctions ne sont pas déclarées présentes dans la candidate 1.1.6. Les données créées pendant les essais doivent être préservées.
