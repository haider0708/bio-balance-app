...
L’hôte partagé dispose d’environ **29 Go libres, 82 % occupés** à la fin de l’installation. Quotas initiaux : médias 4 Gio au total, images par magasin 256 Mio, sauvegardes 4 Gio, réserve disque 10 Gio. Les vidéos réservent jusqu’à 2 Gio plus leur source avant traitement ; le premier réglage à 2 Gio a été corrigé avant la recette finale. Aucune donnée d’un autre service n’a été supprimée.

La rétention automatique ajoutée à la demande de l’utilisateur remplace la conservation illimitée initiale : jusqu’à 7 copies quotidiennes et 4 hebdomadaires, sous le plafond global de 4 Gio et sans supprimer la dernière copie valide. Les flux en cours sont plafonnés et les fichiers partiels sont nettoyés ; voir le [runbook](runbook.md). Les preuves d’installation initiale restent un historique antérieur à cette évolution.
...
Le raffinement visuel suivant utilise désormais les images `biobalance-api:2b3769b` / `biobalance-media:2b3769b`, source `2b3769bad8a78969c3eeb0a8c93ef427a72d241d`. Sauvegarde réussie avant remplacement séquentiel, 15/16 couches comparées, six services sains et modèle compilé du worker vérifié par SHA-256. Aucun changement de schéma ni nouvel envoi externe. Voir [la preuve du raffinement](../tests/deployment/email-refinement-evidence-2026-09-22.json).
